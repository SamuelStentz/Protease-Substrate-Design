...
def main(args):
	...
	for (i, j) in itertools.product(*[amino_acids, amino_acids]):
		# make our pose for this one run
		pose = orig_pose.clone()

		substrate = first_three + i + j
		pose = make_residue_changes(pose, sf, substrate, 
		...
		pose.pdb_info().name("substrate."+substrate)
	...
	print("Time to make silent file: {}".format(time.time() - initial_time))	
